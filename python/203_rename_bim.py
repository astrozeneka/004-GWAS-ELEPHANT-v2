...
if __name__ == '__main__':
    data = open("../data/plink_ema_hair_chr123/snp.hair-old.bim").read().strip().split("\n")
    data = [a.split("\t") for a in data]
    chromosome_list = list(set([a[0] for a in data]))
    chromosome_list = [a for a in chromosome_list if "CM" in a]
    ...
    with open("../data/plink_ema_hair_chr123/snp.hair.bim", "w") as f:
        f.write(output)
    print("Done")