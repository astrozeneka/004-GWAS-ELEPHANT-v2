...
if __name__ == '__main__':
    snp_data = pd.read_csv("../data/80x_plink_output/maf01/maf01.bim", sep="\t", header=None)
    snp_data = snp_data.iloc[:, [0, 3]]
    snp_data.columns = ["chr", "bp"]
    snp_data = snp_data[snp_data['chr'].str.contains("CM")]
    snp_data["chr"] = snp_data["chr"].apply(lambda x:"em"+chromosome_dict[x])
    snp_data.to_csv("../data/80x_plink_output/maf01/maf01.raw.txt", sep="\t", header=None)
    print("Done")
    # เปลียนชื่อ chromosome
    # ไม่ต้องการ chromosome_offset

    # Calculating the sliding window
    snp_density_data = pd.DataFrame(columns=['chr', 'begin', 'end', 'density'])
    for chr in snp_data['chr'].unique():
        chromosome_snps = snp_data[snp_data["chr"] == chr].reset_index(drop=True)
        window_boundaries = np.arange(0, chromosome_snps['bp'].max() + SLIDING_WINDOW, SLIDING_WINDOW)
        chromosome_snps['window'] = pd.cut(chromosome_snps['bp'], bins=window_boundaries, labels=False)
        chromosome_snpperkb = chromosome_snps.groupby('window').size().reset_index(name='density')
        chromosome_snpperkb['begin'] = chromosome_snpperkb['window'].apply(lambda x:int(x*SLIDING_WINDOW))
        chromosome_snpperkb['end'] = chromosome_snpperkb['window'].apply(lambda x:int((x+1)*SLIDING_WINDOW))
        chromosome_snpperkb = chromosome_snpperkb.drop(columns=['window'])
        chromosome_snpperkb.loc[:, ['chr']] = chr
        chromosome_snpperkb = chromosome_snpperkb[['chr', 'begin', 'end', 'density']]
        snp_density_data = pd.concat([snp_density_data, chromosome_snpperkb], axis=0)

    snp_density_data = snp_density_data[['chr', 'begin', 'end', 'density']]
    snp_density_data["density"] = snp_density_data["density"] / snp_density_data["density"].max()
    snp_density_data["density"] = snp_density_data["density"].apply(lambda x:round(x, 3))
    snp_density_data.to_csv("../data/80x_plink_output/maf01/maf01.density.txt", sep="\t", header=None, index=False)
    print("Done")