...
if __name__ == '__main__':
    data = pd.read_csv("../data/101_gemma/hair/gemma_lmm1.assoc.txt", sep="\t")
    data = data[['chr', 'ps', 'p_wald']]
    data["option"] = None
    data["ps_end"] = data["ps"]
    data = data[["chr", "ps", "ps_end", "p_wald", "option"]]
    data = data[data["chr"].str.contains("CM")]
    data["chr"] = data["chr"].apply(lambda x: "em" + chromosome_dict[x])
    data["p_wald"] = data["p_wald"].apply(lambda x: np.log10(x))

    sig_thres = np.log10(0.05 / len(data))
    pot_thres = np.log10(1 / len(data))
    ...
    output["ns"] = data[data["p_wald"] > pot_thres]
    output["pot"] = data[(data["p_wald"] < pot_thres) & (data["p_wald"] > sig_thres)]
    output["sig"] = data[data["p_wald"] < sig_thres]
    for key in output:
        output[key].to_csv(f"../data/80x_plink_output/maf01/snps-{key}.csv", sep="\t", header=None, index=False)
        print(f"Write {key}")
    print("Done")
